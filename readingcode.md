<!-- TOC -->
- [1. run_qsym_afl.py main](#1-run_qsym_aflpy-main)    
    - [1.1. parse_args() 解析命令 run_qsym.py](#11-parse_args-解析命令-run_qsympy)    
    - [1.2. AFLExecutor 构造](#12-aflexecutor-构造)
    - [1.3. e.run 执行](#13-erun-执行)
        - [1.3.1. sync_files 与fuzzer同步文件](#131-sync_files-与fuzzer同步文件)
        - [1.3.2. run_file 测试样本](#132-run_file-测试样本)
            - [1.3.2.1. check_so_file() 检查pintool so](#1321-check_so_file-检查pintool-so)
            - [1.3.2.2. self.run_target()  运行](#1322-selfrun_target--运行)
            - [1.3.2.3. self.handle_by_return_code(ret, fp) 处理返回码,分类保存测试样本](#1323-selfhandle_by_return_coderet-fp-处理返回码分类保存测试样本)
            - [1.3.2.4. q.get_testcases(): 获取测试例](#1324-qget_testcases-获取测试例)
            - [1.3.2.5. self.minimizer.check_testcase(testcase): 检查测试例](#1325-selfminimizercheck_testcasetestcase-检查测试例)
                - [1.3.2.5.1. self.is_interesting_testcase(this_bitmap, proc.returncode)检查是否interesting](#13251-selfis_interesting_testcasethis_bitmap-procreturncode检查是否interesting)
            - [1.3.2.6. self.check_crashes() 同步崩溃样本序号](#1326-selfcheck_crashes-同步崩溃样本序号)
- [2. self.run_target()  运行 1.2.2.2](#2-selfrun_target--运行-1222)
    - [2.1. executor.Executor(self.cmd, self.cur_input, self.tmp_dir, bitmap=self.bitmap, argv=["-l", "1"]) Executor构造](#21-executorexecutorselfcmd-selfcur_input-selftmp_dir-bitmapselfbitmap-argv-l-1-executor构造)
    - [2.2. q.run(self.state.timeout) 运行](#22-qrunselfstatetimeout-运行)
        - [2.2.1. self.gen_cmd(timeout) pin命令行](#221-selfgen_cmdtimeout-pin命令行)
        - [2.2.2. ExecutorResult](#222-executorresult)
- [3. pintool main](#3-pintool-main)
    - [3.1. checkOpt())  参数](#31-checkopt--参数)
    - [3.2. hookSyscalls 系统调用](#32-hooksyscalls-系统调用)
        - [3.2.1. initializeSyscallDesc(); 初始化](#321-initializesyscalldesc-初始化)
        - [3.2.2. kFdSet.insert(STDIN_FILENO);  hook stdin](#322-kfdsetinsertstdin_fileno--hook-stdin)
        - [3.2.3. setSocketCallHook(); hook net](#323-setsocketcallhook-hook-net)
        - [3.2.4. setMMapHookForFile();  hook fs](#324-setmmaphookforfile--hook-fs)
    - [3.3. initializeGlobalContext(  建立上下文](#33-initializeglobalcontext--建立上下文)
        - [3.3.1. g_solver = new Solver(input, out_dir, bitmap);  solver类](#331-g_solver--new-solverinput-out_dir-bitmap--solver类)
            - [3.3.1.1. checkOutDir();  检查输出目录是否合法](#3311-checkoutdir--检查输出目录是否合法)
            - [3.3.1.2. readInput();  读入输入](#3312-readinput--读入输入)
        - [3.3.2. g_expr_builder = SymbolicExprBuilder::create();  符号执行表达式](#332-g_expr_builder--symbolicexprbuildercreate--符号执行表达式)
            - [3.3.2.1. create ??](#3321-create-)
    - [3.4. initializeQsym();初始化](#34-initializeqsym初始化)
        ...
        - [3.4.3. PIN_AddSyscallEntryFunction(onSyscallEnter, NULL); 系统调用进入](#343-pin_addsyscallentryfunctiononsyscallenter-null-系统调用进入)
        - [3.4.4. PIN_AddSyscallExitFunction(onSyscallExit, NULL); 系统调用退出](#344-pin_addsyscallexitfunctiononsyscallexit-null-系统调用退出)
        - [3.4.5. TRACE_AddInstrumentFunction(analyzeTrace, NULL); 基本块插桩](#345-trace_addinstrumentfunctionanalyzetrace-null-基本块插桩)
            - [3.4.5.1. analyzeBBL(bbl); 基本块级分析](#3451-analyzebblbbl-基本块级分析)
                - [3.4.5.1.1. instrumentBBL 分析代码](#34511-instrumentbbl-分析代码)
            - [3.4.5.2. analyzeInstruction(ins); 指令级分析](#3452-analyzeinstructionins-指令级分析)
                - [3.4.5.2.1. cmp  analyzeBinary(ins, Sub, false);](#34521-cmp--analyzebinaryins-sub-false)
                    - [3.4.5.2.1.1. instrumentBinaryRegReg](#345211-instrumentbinaryregreg)
                    - [3.4.5.2.1.2. ExprRef expr_res = doBinary(thread_ctx, kind, op_kind, expr_dst, expr_src); 运算](#345212-exprref-expr_res--dobinarythread_ctx-kind-op_kind-expr_dst-expr_src-运算)
                    - [3.4.5.2.1.3. thread_ctx->setExprToReg(dst, expr_res); 赋值](#345213-thread_ctx-setexprtoregdst-expr_res-赋值)
                - [3.4.5.2.2. jz analyzeJcc(ins, JCC_Z, false);](#34522-jz-analyzejccins-jcc_z-false)
                - [3.4.5.2.3. mov](#34523-mov)
        - [3.4.6. PIN_AddInternalExceptionHandler(exceptionHandler, NULL); 异常处理](#346-pin_addinternalexceptionhandlerexceptionhandler-null-异常处理)
- [4. jz指令分析 analyzeJcc(ins, JCC_Z, false);](#4-jz指令分析-analyzejccins-jcc_z-false)
    - [4.1. instrumentJcc](#41-instrumentjcc)
        - [4.1.1. ExprRef e = thread_ctx->computeJcc(ctx, jcc_c, inv);计算jz表达式](#411-exprref-e--thread_ctx-computejccctx-jcc_c-inv计算jz表达式)
            - [4.1.1.1. computeFastJcc](#4111-computefastjcc)
            - [4.1.1.2. computeSlowJcc](#4112-computeslowjcc)
                - [4.1.1.2.1. e = computeFlag(EFLAGS_ZF); jz](#41121-e--computeflageflags_zf-jz)
                - [4.1.1.2.2. return flag_op->computeZF(); //liu](#41122-return-flag_op-computezf-liu)
        - [4.1.2. g_solver->addJcc(e, taken, pc);  求解](#412-g_solver-addjcce-taken-pc--求解)
            - [4.1.2.1. is_interesting = isInterestingJcc(e, taken, pc); //liu 是感兴趣的吗？](#4121-is_interesting--isinterestingjcce-taken-pc-liu-是感兴趣的吗)
            - [4.1.2.2. negatePath(e, taken); //liu 翻转分支点](#4122-negatepathe-taken-liu-翻转分支点)
                - [4.1.2.2.1. addToSolver(e, !taken);//liu](#41221-addtosolvere-takenliu)
                - [4.1.2.2.2. bool sat = checkAndSave();//liu](#41222-bool-sat--checkandsaveliu)
                    - [4.1.2.2.2.1. if (check() == z3::sat) {//liu](#412221-if-check--z3sat-liu)
                    - [4.1.2.2.2.2. saveValues(postfix); //liu 保存求解值](#412222-savevaluespostfix-liu-保存求解值)
                    - [4.1.2.2.2.3. std::vector<UINT8> values = getConcreteValues();//liu solve](#412223-stdvectoruint8-values--getconcretevaluesliu-solve)
            - [4.1.2.3. addConstraint(e, taken, is_interesting); //liu 添加约束](#4123-addconstrainte-taken-is_interesting-liu-添加约束)
- [5. pintool调试](#5-pintool调试)
    - [5.1. run_qsym.py脚本](#51-run_qsympy脚本)
    - [5.2. pin命令，5.1打印出的](#52-pin命令51打印出的)
    - [5.3. pinbin gdb](#53-pinbin-gdb)
    - [5.4. 编译](#54-编译)
- [6. z3 solver的例子](#6-z3-solver的例子)
    - [6.1. github sample](#61-github-sample)
- [7. read 污点引入](#7-read-污点引入)
    - [7.1. g_memory.makeExpr(ctx->arg[SYSCALL_ARG1], ctx->ret);//liu 符号化](#71-g_memorymakeexprctx-argsyscall_arg1-ctx-retliu-符号化)
        - [7.1.1. LOG_DEBUG](#711-log_debug)
        - [7.1.2. ExprRef e = g_expr_builder->createRead(off_++); //liu 建立符号名称](#712-exprref-e--g_expr_builder-createreadoff_-liu-建立符号名称)
        - [7.1.3. setExprToMem(addr, e);//liu 存入内存](#713-setexprtomemaddr-eliu-存入内存)
            - [7.1.3.1. clearExprFromMem(addr);//liu 清除](#7131-clearexprfrommemaddrliu-清除)
            - [7.1.3.2. *getExprPtrFromMem(addr) = e;//liu 赋值](#7132-getexprptrfrommemaddr--eliu-赋值)
                - [7.1.3.2.1. ExprRef* page = getPage(addr); //liu 找到页面](#71321-exprref-page--getpageaddr-liu-找到页面)
                - [7.1.3.2.2. return &page[addressToOffset(addr)]; //liu 找到页面里偏移](#71322-return-pageaddresstooffsetaddr-liu-找到页面里偏移)
    - [7.2. 从求解反推](#72-从求解反推)
        - [7.2.1. getConcreteValues](#721-getconcretevalues)
        - [7.2.2. readInput初始化](#722-readinput初始化)
        - [7.2.3. solver类构造函数](#723-solver类构造函数)
        - [7.2.4. 初始化input_file](#724-初始化input_file)
        - [7.2.5. 在pintool 的main函数中初始化](#725-在pintool-的main函数中初始化)
<!-- /TOC -->
